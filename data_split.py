...
df_filtered.to_csv("qogita_filtered_products2.csv", index=False)

# Read csv file
df = pd.read_csv("qogita_filtered_products2.csv")
...
df_filtered = df_funcs.filter_categories_with_min_products("qogita_filtered_products.csv", 50)

# Split the dataset into train, validation, and test sets
df_train, df_val, df_test = df_funcs.split_dataset(df_filtered, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1)

# Save the filtered DataFrame to a new CSV file
df_train.to_csv("train2.csv", index=False)
df_val.to_csv("validation2.csv", index=False)
